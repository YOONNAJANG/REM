...
###BART large focus
#CUDA_VISIBLE_DEVICES=0 python train_refiner.py --ner_coef 0.3 --epochs 1 --pretrained_model facebook/bart-large --data_type focus --train_batch_size 8 --grad_accum 32 --lr 6.25e-5 --output_dir ${output_dir}/bart_large_focus --train_path ${focus_train_path} --train_cache_path ${focus_train_cache_path} --dev_path ${focus_dev_path} --dev_cache_path ${focus_dev_cache_path}
###BART large wow
CUDA_VISIBLE_DEVICES=0 python train_refiner.py --ner_coef 0.7 --epochs 100 --pretrained_model facebook/bart-large --data_type wow --train_batch_size 8 --grad_accum 32 --lr 6.25e-5 --output_dir ${output_dir}/bart_large_wow --train_path ${wow_train_path} --train_cache_path ${wow_train_cache_path} --dev_path ${wow_dev_path} --dev_cache_path ${wow_dev_cache_path}
###BART large cmudog
#CUDA_VISIBLE_DEVICES=0 python train_refiner.py --ner_coef 0.5 --epochs 100 --pretrained_model facebook/bart-large --data_type cmudog --train_batch_size 8 --grad_accum 32 --lr 6.25e-5 --output_dir ${output_dir}/bart_large_cmudog --train_path ${cmu_train_path} --train_cache_path ${cmu_train_cache_path} --dev_path ${cmu_dev_path} --dev_cache_path ${cmu_dev_cache_path}